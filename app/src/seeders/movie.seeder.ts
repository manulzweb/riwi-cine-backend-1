...
const seed = async () => {
  ...
  const movie1 = await Movie.create({
    ...
    synopsis: "Un grupo de exploradores debe viajar entre líneas temporales para evitar una catástrofe.",
  ...
  const movie3 = await Movie.create({
    ...
    synopsis: "Una odisea espacial sobre una tripulación que busca un nuevo hogar para la humanidad.",
    director: "Ana Restrepo",
  ...
  await MovieFunction.bulkCreate([
    ...
    { movieId: movie1.id, dateTime: inTwoDays, format: "2D", room: "Sala 1", price: 12000, totalSeats: 80, availableSeats: 40 },
    { movieId: movie1.id, dateTime: inTwoDays, format: "IMAX", room: "Sala IMAX", price: 22000, totalSeats: 100, availableSeats: 0 }, // agotada -> soldOut true
    { movieId: movie2.id, dateTime: inTwoDays, format: "2D", room: "Sala 2", price: 11000, totalSeats: 60, availableSeats: 60 },
    { movieId: movie3.id, dateTime: inTwoDays, format: "VIP", room: "Sala VIP", price: 28000, totalSeats: 40, availableSeats: 10 },

    // Función pasada (NO debe aparecer en /functions, valida RN-014)
    { movieId: movie1.id, dateTime: yesterday, format: "2D", room: "Sala 1", price: 12000, totalSeats: 80, availableSeats: 5 },
  ]);
